...
Σε αυτό το εργαστήριο χρησιμοποιούμε **Hardware Interrupts** για ακαριαία απόκριση σε εξωτερικά ερεθίσματα (πατήματα κουμπιών), και διαβάζουμε αναλογικές τιμές από τον αισθητήρα θερμοκρασίας TMP36.
...
Χρήση της `attachInterrupt()` στο Pin 2. Με κάθε πάτημα του κουμπιού αλλάζει ακαριαία το χρώμα του RGB LED (από κίτρινο σε μωβ). [cite_start]Οι μεταβλητές των χρωμάτων δηλώθηκαν ως `volatile`
...
Ανάγνωση θερμοκρασίας κάθε 2.5 δευτερόλεπτα με χρήση της `millis()` (non-blocking). Το RGB LED δείχνει Κόκκινο για > 25°C, Μπλε για < 5°C, και Κίτρινο για ενδιάμεσες θερμοκρασίες.
...
Συνδυασμός των δύο προηγούμενων. Το Arduino διαβάζει τη θερμοκρασία κάθε 2.5s. Αν όμως ο χρήστης πατήσει το κουμπί (Interrupt), το σύστημα κάνει ακαριαία νέα μέτρηση, ανανεώνει το LED και μηδενίζει το χρονόμετρο της `millis()`.
